...
export async function startShell() {
    ...
    if (hasProot) {
        ...
        const mode = await p.select({
            ...
            options: [
                { value: 'Arch Terminal', label: 'Normal Arch Terminal', hint: 'Default proot-distro' },
                { value: 'Arch XTerm', label: 'XTerminal', hint: 'Modded Arch session by XTermLite' },
...
EOF
                ...
                child = spawn('proot-distro', ['login', 'archlinux', '--', 'bash', '-lc', tmuxScript], {
                    stdio: 'inherit',
                    shell: true
                });
            } else {
                child = spawn('proot-distro', ['login', 'archlinux'], {
                    stdio: 'inherit',
                    shell: true
                });
            }
